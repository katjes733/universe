...
public class Object_3D extends TransformGroup {
	...
	private static final String PARENT_PATH_MODELS = "models/";

	public Object_3D(final String filenameModel) {
		loader = new Inspector3DS(getAbsolutePathToReourceFile(filenameModel));
	...
	public Object_3D(final String filenameModel, final double ScaleFactor) {
		loader = new Inspector3DS(getAbsolutePathToReourceFile(filenameModel));
		loader.setTextureLightingOn(); // turns on modulate mode for textures (lighting)
		loader.parseIt();
		theModel = loader.getModel();
	...
	public Object_3D(final String filenameModel, final double ScaleFactorX,
	...
	private String getAbsolutePathToReourceFile(
		...
		final String returnValue = getClass().getClassLoader()
				.getResource(PARENT_PATH_MODELS + relativePathToResourceFile)
				.getPath();
		return returnValue;
	}
}
